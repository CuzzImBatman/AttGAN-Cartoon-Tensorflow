...
val_iter =tf.compat.v1.data.make_one_shot_iterator(val_dataset)
...
#         b = tf.random_shuffle(a)
        ...
        step = tf.compat.v1.train.AdamOptimizer(lr, beta1=args.beta_1).minimize(loss, global_step=step_cnt, var_list=D.func.trainable_variables)

        # summary
        with tf.summary.create_file_writer('./output/%s/summaries/D' % args.experiment_name).as_default(),\
                 tf.summary.record_if(step_cnt%10==0):
            summary = [
                tl.summary_v2({
...
def G_train_graph():
    ...
    # placeholders & inputs
        ...
        # optim
        step_cnt, _ = tl.counter()
        step =  tf.compat.v1.train.AdamOptimizer(lr, beta1=args.beta_1).minimize(loss, global_step=step_cnt, var_list=Genc.func.trainable_variables + Gdec.func.trainable_variables)

        # summary
        with tf.summary.create_file_writer('./output/%s/summaries/G' % args.experiment_name).as_default(),\
                    tf.summary.record_if(step_cnt%10==0):
